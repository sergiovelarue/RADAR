  ...
  try{
    ...
    r.innerHTML = `<strong>Validación correcta:</strong>
      <div class="summary-grid">
        ...
        <article><span>Registros Colchones</span><strong>${col.rows.length}</strong></article>
        ...
        <article><span>Venta Colchones ${current}</span><strong>${money(colCurrent)}</strong></article>
      </div>
      <p>${esp.fileName} · ${col.fileName}</p>`;
  }catch(err){
    r.className = "update-result error";
    r.innerHTML = `<strong>Error:</strong> ${err.message}`;
  }
